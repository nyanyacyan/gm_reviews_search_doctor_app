...
class NearbySearchService {
  static Future<List<Map<String, dynamic>>> fetchNearbyHospitals({
    ...
    logDebug('APIのレスポンス: $data'); // レスポンスのログ出力
    if (data['status'] != "OK") {
    ...
    logDebug('rawResultsの型: ${rawResults.runtimeType}'); // dynamic か List?

    // 2. Listとして扱えるように変換
    final List<dynamic> resultList =
        rawResults as List; // json.decodeした際にはこの箇所で as List<dynamic>にて明示する必要がある
    logDebug('resultList数: ${resultList.length}');
    logDebug(
      'resultListの1件目: ${resultList.isNotEmpty ? resultList[0] : '空リスト'}',
    );
